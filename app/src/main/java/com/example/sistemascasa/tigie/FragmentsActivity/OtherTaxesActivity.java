...
import com.example.sistemascasa.tigie.adapter.OtherAdapter;
...
public class OtherTaxesActivity extends AppCompatActivity {

    private Toolbar toolbar;
    public TabLayout tabLayout;
    private Context context;
    private Activity activity;
    TextView tvFraccionOtherCode;
    private Integer valTigie;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_other_taxes);
        ...
        tvFraccionOtherCode = (TextView) findViewById(R.id.tvFraccionOtherCode);
        tvFraccionOtherCode.setText(fractionCode);

        getOtherTaxesWs(id_fraccion, valTigie);
    }

    public void getOtherTaxesWs (Integer id_fraccion, Integer valTigie) {
        //fraccion = "19012001";
        try {
            ...
            Call<ArrayList<OtherTaxes>> tlcCall = tlcService.getothertaxes(id_fraccion, valTigie, email, token);

            tlcCall.enqueue(new Callback<ArrayList<OtherTaxes>>() {
                @Override
                public void onResponse(Call<ArrayList<OtherTaxes>> call, Response<ArrayList<OtherTaxes>> response) {
                    try {
                        if (response.body() == null) {
                            mandarErrorOther ();
                        } else {
                            ArrayList<OtherTaxes> otherTaxesArrayList = response.body();
                            if (otherTaxesArrayList.size() > 0) {
                                RecyclerView rvC = (RecyclerView) findViewById(R.id.rvOtherTaxes);

                                LinearLayoutManager llm = new LinearLayoutManager(context);
                                rvC.setLayoutManager(llm);

                                OtherAdapter adapter = new OtherAdapter(otherTaxesArrayList, activity);
                ...
                @Override
                public void onFailure(Call<ArrayList<OtherTaxes>> call, Throwable t) {
                    mandarErrorOther ();
                }
            });
        } catch (IOError e) {
    ...
    public void mandarErrorOther() {
        Toast.makeText(OtherTaxesActivity.this , "¡ Fallo de Conexión ! ... Inténtelo más tarde, por favor.", Toast.LENGTH_LONG).show();
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.mHeadings:
                Intent intent_close = new Intent(OtherTaxesActivity.this, SubheadingActivity.class);
            ...
            case R.id.mSubheadings:
                Intent intent_sh = new Intent(OtherTaxesActivity.this, FractionsActivity.class);
